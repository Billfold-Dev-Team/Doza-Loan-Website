...
<div class="wrapperform" id="popuphome">
	<div class="modal_box">
		...
		<div class="modal_box_wrap">
			<div class="modal_close" onclick="hide('popuphome')"></div>
			<form method="post" action="./handlers/process.php">
			<div class="modal_header">
				Hello, <?php echo $fetch_info['firstName'] ?>
                    <?php
                    if(count($errors) > 0){
                        ?>
			...
			<div class="modal_body">
				<p>Welcome to <span style="color:#339900;">Doza.</span></p>
				<p>You are eligible for a ZMW 500 loan from Doza.</p>
				<p></p><br>
				<p> <input type="number"  id="principle" placeholder="Enter Amount" style="outline:none; border-bottom:3px solid #339900;border-left:3px solid #339900;border-top:none;border-right:none;border-radius:3px;" name="principle"> </p>
			</div>
			<div class="modal_footer">
				<div class="modal_btn_grp">
				<input type="hidden" id="loan_ref" name="loan_ref" value="<?php echo $fetch_info['nrcNumber'] ?>">
	                        <input type="hidden" id="" name="email" value="<?php echo $fetch_info['email'] ?>">
				<input type="hidden" id="loan_product" name="loan_product" value="individual">
				<input type="hidden" id="interest_rate" name="interest_rate" value="10%">
				<input type="hidden" id="loan_interest" name="loan_interest" value="25%">
				<input type="hidden" id="loan_toatal_amount" name="loan_total_amount" value="K1000">
				<input type="hidden" id="loan_type" name="loan_type" value="individial">
				    
					<div><input type="submit" id="apply" name="getloan" value="Apply" class="btn btn_cancel"></div>
		...
		$('#apply').click(function(e){
			...
			var loan_status 	= $('#loan_status').val();
				...
				$.ajax({
					type: 'POST',
					url: 'process.php',
					data: {loan_ref: loan_ref,loan_product: loan_product,interest_rate: interest_rate,principle: principle,loan_interest: loan_interest,loan_total_amount:loan_total_amount,loan_type:loan_type,loan_status:loan_status},
					...
					error: function(data){
						Swal.fire({
								...
								'text': 'Check your Email for a Loan verification code and wait for approval.',
								'type': 'success'
								})
					}
				});
